...
# - uses glance to obtain the R2 value and stores it in r.squared
    r.squared[nm] <- glance(fit)$r.squared
}
```

```{r for-loop-end}
# Use which.max() to find the best predictor for mpg 
m <- which.max(r.squared)
...
```{r documentation}
library(docstring)
source("solutions/functions.R")
?qq_norm
```

```{r tests}
library(testthat)
test_file("solutions/tests.R")
```
